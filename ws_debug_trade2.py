#!/usr/bin/env python3
"""
Decode TRADE_EVENT properly (Pp schema = action_id + Op trade_request + Ap trade_result)
and place correct orders.
"""
...
# Op schema (trade request) - 248 bytes
OP_SCHEMA = [
    {'propType':6},          # [0] action_id
    {'propType':6},          # [1] trade_action
    {'propType':11,'propLength':64},   # [2] symbol
    {'propType':18},         # [3] volume
    {'propType':6},          # [4] digits
    {'propType':18},         # [5] trade_order
    {'propType':6},          # [6] trade_type
    {'propType':6},          # [7] type_filling
    {'propType':6},          # [8] type_time
    {'propType':6},          # [9] type_flags
    {'propType':6},          # [10] type_reason
    {'propType':8},          # [11] price_order
    {'propType':8},          # [12] price_trigger
    {'propType':8},          # [13] price_sl
    {'propType':8},          # [14] price_tp
    {'propType':6},          # [15] price_deviation
    {'propType':8},          # [16] price_top
    {'propType':8},          # [17] price_bottom
    {'propType':11,'propLength':64},   # [18] comment
    {'propType':18},         # [19] trade_position
    {'propType':18},         # [20] position_by
    {'propType':6},          # [21] time_expiration
]
OP_SIZE = series_size(OP_SCHEMA)

# Ap schema (trade result) - 128 bytes
AP_SCHEMA = [
    {'propType':6},          # [0] retcode
    {'propType':17},         # [1] deal
    {'propType':17},         # [2] trade_order
    {'propType':17},         # [3] volume
    {'propType':8},          # [4] price
    {'propType':6},          # [5] res_time
    {'propType':8},          # [6] bid
    {'propType':8},          # [7] ask
    {'propType':8},          # [8] last
    {'propType':11,'propLength':64},   # [9] comment
]
AP_SIZE = series_size(AP_SCHEMA)
...
ERROR_CODES = {
    ...
    5: 'server problem', 6: 'server problem', 7: 'network problem',
    8: 'permissions', 9: 'timeout',
    10009: 'done', 10010: 'done', 10011: 'common error',
    10012: 'timeout', 10013: 'invalid parameters', 10014: 'invalid volume',
    ...
    10042: 'buy not allowed', 10043: 'sell not allowed', 10044: 'trade not allowed',
    11001: 'done',
}
...
def decode_trade_event(body):
    """Decode 380B TRADE_EVENT: [action_id(u32), Op request(248B), Ap result(128B)]"""
    if len(body) < 380:
        print(f"  [!] Short TRADE_EVENT: {len(body)}B (expected 380)")
        return None

    action_id = struct.unpack_from('<I', body, 0)[0]
    print(f"  [0-3]   action_id = {action_id}")

    # Decode trade request (Op schema at offset 4)
    op_vals, _ = parse_series(body[4:], OP_SCHEMA, 0)
    print(f"  Trade request:")
    print(f"    trade_action = {op_vals[1]}")
    print(f"    symbol = '{op_vals[2]}'")
    print(f"    volume = {op_vals[3]} ({op_vals[3]/100000000:.2f} lots)")
    print(f"    trade_type = {op_vals[6]} ({TRADE_TYPES.get(op_vals[6], '?')})")
    print(f"    price_order = {op_vals[11]:.5f}")
    print(f"    price_sl = {op_vals[13]:.5f}")
    print(f"    price_tp = {op_vals[14]:.5f}")
    print(f"    comment = '{op_vals[18]}'")

    # Decode trade result (Ap schema at offset 4+OP_SIZE)
    ap_offset = 4 + OP_SIZE
    ap_vals, _ = parse_series(body[ap_offset:], AP_SCHEMA, 0)
    retcode = ap_vals[0]
    error_msg = ERROR_CODES.get(retcode, f'unknown({retcode})')
    print(f"  Trade result:")
    print(f"    retcode = {retcode} ({error_msg})")
    print(f"    deal = {ap_vals[1]}")
    print(f"    trade_order = {ap_vals[2]}")
    print(f"    volume = {ap_vals[3]}")
    print(f"    price = {ap_vals[4]:.5f}")
    print(f"    bid = {ap_vals[6]:.5f}")
    print(f"    ask = {ap_vals[7]:.5f}")

    return {'action_id': action_id, 'retcode': retcode, 'error': error_msg,
            'op_vals': op_vals, 'ap_vals': ap_vals}
...
async def main():
    ...
    async with websockets.connect(WS_URL, ssl=ssl_ctx, ping_interval=None,
        ...
        # Get symbol spec for stops_level, freeze_level
        await send_cmd(ws, sk, 17, struct.pack('<I', sym_id))
        r17 = await send_and_wait(ws, sk, 17, struct.pack('<I', sym_id), 17, timeout=5)
        stops_level = 0
        freeze_level = 0
        if r17:
            body17 = r17['res_body']
            if len(body17) >= 24:
                # Symbol spec response: parse key fields
                # stops_level at some offset, freeze_level at another
                print(f"  Symbol spec: {len(body17)}B hex={body17[:30].hex()}")
                # From the schema: stops_level, freeze_level are in the trade config
                # Let me try to find them
                if len(body17) >= 28:
                    stops_level = struct.unpack_from('<I', body17, 20)[0]
                    freeze_level = struct.unpack_from('<I', body17, 24)[0]
                    print(f"  stops_level={stops_level} freeze_level={freeze_level}")

        # ===== TEST: Proper BUY STOP order =====
        print(f"\n{'='*60}")
        limit_price = current_price + (stops_level + 10) * 10**(-digits)
        print(f"TEST: BUY STOP 0.01 {target} @ {limit_price:.5f}")
        print(f"  (current={current_price:.5f}, stops_level={stops_level} points)")
        print(f"{'='*60}")
        ...
        struct.pack_into('<I', op, 92, 4)                   # trade_type = BUY_STOP
        ...
        struct.pack_into('<d', op, 112, limit_price)        # price
        ...
        if r:
            ...
            print(f"\n  Positions: {len(positions)}, Pending orders: {len(orders)}")
            for o in orders:
                state = {0:'STARTED', 2:'CANCELED', 4:'FILLED', 5:'REJECTED'}.get(o[17], f"?{o[17]}")
                print(f"  Order: {o[2]} type={TRADE_TYPES.get(o[6],'?')} state={state} price={o[10]:.5f} vol={o[15]/100000000:.2f}")

        # ===== TEST: BUY LIMIT below market =====
        print(f"\n{'='*60}")
        limit_price2 = current_price - (stops_level + 10) * 10**(-digits)
        print(f"TEST: BUY LIMIT 0.01 {target} @ {limit_price2:.5f}")
        print(f"  (current={current_price:.5f}, {stops_level+10} points below)")
        print(f"{'='*60}")

        op2 = bytearray(OP_SIZE)
        struct.pack_into('<I', op2, 0, 0)
        struct.pack_into('<I', op2, 4, 1)                    # PENDING
        op2[8:8+len(sym_bytes)] = sym_bytes
        struct.pack_into('<Q', op2, 72, 1000)                # 0.01 lots
        struct.pack_into('<I', op2, 80, digits)
        struct.pack_into('<Q', op2, 84, 0)
        struct.pack_into('<I', op2, 92, 2)                    # BUY_LIMIT
        struct.pack_into('<I', op2, 96, 0)
        struct.pack_into('<I', op2, 100, 0)
        struct.pack_into('<I', op2, 104, 2)
        struct.pack_into('<I', op2, 108, 0)
        struct.pack_into('<d', op2, 112, limit_price2)
        struct.pack_into('<d', op2, 120, 0)
        struct.pack_into('<d', op2, 128, 0)
        struct.pack_into('<d', op2, 136, 0)
        struct.pack_into('<I', op2, 144, 0)
        struct.pack_into('<d', op2, 148, 0)
        struct.pack_into('<d', op2, 156, 0)
        struct.pack_into('<Q', op2, 228, 0)
        struct.pack_into('<Q', op2, 236, 0)

        t0 = time.time()
        await send_cmd(ws, sk, 12, bytes(op2))
        ...
        for m in msgs:
            ...
            if m['cmd_id'] == 12:
                ...
                print(f"  [{t:.3f}s] TRADE_EVENT:")
                result = decode_trade_event(m['res_body'])
