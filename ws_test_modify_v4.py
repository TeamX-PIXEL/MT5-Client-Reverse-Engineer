#!/usr/bin/env python3
"""
Test modify with deal ticket from TRADE_EVENT + longer wait.
"""
...
async def main():
    ...
    async with websockets.connect(WS_URL, ssl=ssl_ctx, ping_interval=None,
        ...
        # Wait 5 seconds for position to be fully created
        print("  Waiting 5 seconds...")
        await asyncio.sleep(5)

        # Check positions
        positions = await get_positions(ws, sk)
        print(f"\n=== POSITIONS after 5s: {len(positions)} ===")
        for p in positions:
        ...
        # Try 1: Modify using deal_id at pos_id (offset 228)
        print(f"\n=== Try 1: Modify deal_id={deal_id} at pos_id ===")
        op = mk_trade_op(200, 'EURUSDm', vol1, 5, 0, bid, sl=new_sl, tp=new_tp,
                         pos_id=deal_id, trade_action=6, type_filling=2)
        r = await send_trade(ws, sk, op)
        print(f"  retcode={r['retcode']} ({RC_NAMES.get(r['retcode'],'?')}) comment='{r['comment']}'")

        # Try 2: Modify using deal_id at order_id (offset 84)
        print(f"\n=== Try 2: Modify deal_id={deal_id} at order_id ===")
        op = mk_trade_op(201, 'EURUSDm', vol1, 5, 0, bid, sl=new_sl, tp=new_tp,
                         order_id=deal_id, trade_action=6, type_filling=2)
        r = await send_trade(ws, sk, op)
        print(f"  retcode={r['retcode']} ({RC_NAMES.get(r['retcode'],'?')}) comment='{r['comment']}'")

        # Try 3: Modify using order_id at pos_id
        print(f"\n=== Try 3: Modify order_id={order_id} at pos_id ===")
        op = mk_trade_op(202, 'EURUSDm', vol1, 5, 0, bid, sl=new_sl, tp=new_tp,
                         pos_id=order_id, trade_action=6, type_filling=2)
        r = await send_trade(ws, sk, op)
        print(f"  retcode={r['retcode']} ({RC_NAMES.get(r['retcode'],'?')}) comment='{r['comment']}'")

        # Try 4: Modify using order_id at order_id
        print(f"\n=== Try 4: Modify order_id={order_id} at order_id ===")
        op = mk_trade_op(203, 'EURUSDm', vol1, 5, 0, bid, sl=new_sl, tp=new_tp,
                         order_id=order_id, trade_action=6, type_filling=2)
        r = await send_trade(ws, sk, op)
        print(f"  retcode={r['retcode']} ({RC_NAMES.get(r['retcode'],'?')}) comment='{r['comment']}'")

        # Try 5: If we have a position from cmd_id=4, try that
        if positions:
            pid = positions[-1]['id']
            print(f"\n=== Try 5: Modify cmd_id4_id={pid} at pos_id ===")
            op = mk_trade_op(204, 'EURUSDm', vol1, 5, 0, bid, sl=new_sl, tp=new_tp,
                             pos_id=pid, trade_action=6, type_filling=2)
            r = await send_trade(ws, sk, op)
            print(f"  retcode={r['retcode']} ({RC_NAMES.get(r['retcode'],'?')}) comment='{r['comment']}'")

            # Try 6: Close with cmd_id4_id
            print(f"\n=== Try 6: Close cmd_id4_id={pid} ===")
            op = mk_trade_op(205, 'EURUSDm', vol1, 5, 1, bid, trade_action=3, pos_id=pid)
            r = await send_trade(ws, sk, op)
            print(f"  retcode={r['retcode']} ({RC_NAMES.get(r['retcode'],'?')})")

        # Try 7: Close with deal_id
        print(f"\n=== Try 7: Close deal_id={deal_id} ===")
        op = mk_trade_op(206, 'EURUSDm', vol1, 5, 1, bid, trade_action=3, pos_id=deal_id)
        r = await send_trade(ws, sk, op)
        print(f"  retcode={r['retcode']} ({RC_NAMES.get(r['retcode'],'?')})")

        # Try 8: Close with order_id
        print(f"\n=== Try 8: Close order_id={order_id} ===")
        op = mk_trade_op(207, 'EURUSDm', vol1, 5, 1, bid, trade_action=3, pos_id=order_id)
        r = await send_trade(ws, sk, op)
        print(f"  retcode={r['retcode']} ({RC_NAMES.get(r['retcode'],'?')})")
