#!/usr/bin/env python3
"""
Test order cancellation with trade_action=8 (CancelOrder from C# TradeType enum).
C# enum maps directly to WS trade_action:
  SetOrder=5, CancelOrder=8, ClosePosition=10
"""
...
def mk_op(action_id, trade_action, sym, vol, digits, trade_type, price_order,
    ...
    struct.pack_into('<Q', op, 84, order_ticket)
        ...
        try:
            ...
            if r and r['cmd_id'] == 4:
                ...
                for _ in range(ord_cnt):
                    ...
                    orders.append({'ticket': oid, 'symbol': sym, 'type': otype, 'price': oprice})
                    off += 356
                return orders
        except asyncio.TimeoutError:
            continue
...
async def main():
    ...
    async with websockets.connect(WS_URL, ssl=ssl_ctx, ping_interval=None,
        ...
        # Place 3 test orders
        print(f"\n{'='*60}")
        print("STEP 1: Place test orders")
        print(f"{'='*60}")
        placed = []
        for label, tt, price in [
            ...
            print(f"  {label}: retcode={res['retcode']}({RC.get(res['retcode'],'?')}) order={res['order']}")
            if res['order']: placed.append({'ticket': res['order'], 'type': tt, 'label': label})
            await asyncio.sleep(1)

        await asyncio.sleep(2)

        # =========================================================
        # Try different cancel approaches on each order
        # =========================================================
        print(f"\n{'='*60}")
        print("STEP 2: Try cancellation approaches")
        print(f"{'='*60}")

        for i, order in enumerate(placed):
            ticket = order['ticket']
            print(f"\n--- Cancel #{ticket} ({order['label']}) ---")

            if i == 0:
                # APPROACH 1: trade_action=8 (CancelOrder) + trade_order=ticket
                print(f"  A1: action=8(CancelOrder) type={order['type']} order={ticket}")
                aid = random.randint(1, 0x7FFFFFFE)
                op = mk_op(aid, 8, 'EURUSDm', vol, 5, order['type'], 0.0, order_ticket=ticket)
                res = await send_trade(ws, sk, op)
                print(f"    retcode={res['retcode']}({RC.get(res['retcode'],'?')}) comment='{res['comment']}'")

            elif i == 1:
                # APPROACH 2: trade_action=8 + type_filling=0
                print(f"  A2: action=8 type={order['type']} fill=0 order={ticket}")
                aid = random.randint(1, 0x7FFFFFFE)
                op = mk_op(aid, 8, 'EURUSDm', vol, 5, order['type'], 0.0, order_ticket=ticket, type_filling=0)
                res = await send_trade(ws, sk, op)
                print(f"    retcode={res['retcode']}({RC.get(res['retcode'],'?')}) comment='{res['comment']}'")

            elif i == 2:
                # APPROACH 3: trade_action=8 + trade_type=8 (CANCEL)
                print(f"  A3: action=8 type=8(CANCEL) order={ticket}")
                aid = random.randint(1, 0x7FFFFFFE)
                op = mk_op(aid, 8, 'EURUSDm', vol, 5, 8, 0.0, order_ticket=ticket)
                res = await send_trade(ws, sk, op)
                print(f"    retcode={res['retcode']}({RC.get(res['retcode'],'?')}) comment='{res['comment']}'")
        ...
        # Check remaining
        print(f"\n{'='*60}")
        print("STEP 3: Check remaining orders")
        print(f"{'='*60}")
        orders = await get_pending_orders(ws, sk)
        print(f"  Remaining: {len(orders)} pending orders")
        for o in orders:
            print(f"    #{o['ticket']} {TYPES.get(o['type'],o['type'])} {o['symbol']} price={o['price']:.5f}")

        # Cancel ALL remaining with approach 1
        if orders:
            print(f"\n--- Cancel ALL remaining with action=8 ---")
            for o in orders:
                aid = random.randint(1, 0x7FFFFFFE)
                op = mk_op(aid, 8, 'EURUSDm', vol, 5, o['type'], 0.0, order_ticket=o['ticket'])
                res = await send_trade(ws, sk, op)
                print(f"  #{o['ticket']}: retcode={res['retcode']}({RC.get(res['retcode'],'?')}) comment='{res['comment']}'")
                await asyncio.sleep(1)

        # Final check
        orders2 = await get_pending_orders(ws, sk)
        print(f"\n  Final: {len(orders2)} pending orders remaining")
