#!/usr/bin/env python3
"""
MT5 Broker Search API Client
Implements both Broker.Search() and Broker.SearchMQ() from C# MT5API.

API Endpoints (decrypted from mt5api.dll):
  Search:   http://search.mtapi.io/Search?company={name}&mt5=true  (GET)
  SearchMQ: https://updates.metaquotes.net/public/mt5/network      (POST + HMAC)

SearchMQ signature: MD5(MD5(body) + key)
"""
import hashlib
import json
import random
...
def _compute_searchmq_signature(body: str) -> str:
    """
    SearchMQ HMAC signature: MD5(MD5(body) + key)
    """
...
def _generate_cookie() -> str:
    """
    Generate the cookie header used by SearchMQ.
    Format: _fz_uniq={val};uniq={val};age={age};tid={tid}
    """
    ...
    md5 = hashlib.md5(bytes(rand_bytes)).digest()
    md5 = bytearray(md5)
    md5[0] = 0
    for j in range(1, 16):
...
def search(company: str) -> list:
    """
    Broker.Search() — HTTP GET, no authentication.
    URL: http://search.mtapi.io/Search?company={company}&mt5=true
...
def search_mq(company: str) -> list:
    """
    Broker.SearchMQ() — HTTP POST with HMAC signature.
    URL: https://updates.metaquotes.net/public/mt5/network
    
    Same return format as search().
    """
    body = f"company={company}&code=mt5"
    sig = _compute_searchmq_signature(body)
...
def find_server(server_name: str) -> dict | None:
    ...
    Tries search_mq first, falls back to search.
    
    Returns: {"company": "...", "name": "...", "access": [...], ...} or None
    """
    for api_fn in [search_mq, search]:
        try:
            results = api_fn(server_name)
...
def find_server_ips(server_name: str) -> list[str]:
    """
    Find server IP addresses by name.
    Returns list of "ip:port" strings or empty list.
    """
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python broker_search.py <server_name> [server_name2 ...]")
        print("Example: python broker_search.py Exness-MT5Trial17 Exness-MT5Trial16")
        sys.exit(1)

    for name in sys.argv[1:]:
        print(f"\n=== {name} ===")
        info = find_server(name)
        if info:
            print(f"  Company:  {info['company']}")
            print(f"  Site:     {info['site']}")
            print(f"  Demo:     {'Yes' if info['is_demo'] else 'No'}")
            print(f"  Access:   {', '.join(info['access'])}")
        else:
            print("  NOT FOUND")
